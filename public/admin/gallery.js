(() => {
  ...
  function describeSelection(files) {
    ...
    if (list.length === 0) return "Choose photos";
    if (list.length === 1) return list[0].name;
    return `${list.length} photos selected`;
  }
  ...
  fileInput?.addEventListener("change", () => {
    ...
    if (fileName) {
      ...
      fileName.parentElement?.classList.toggle("has-file", (fileInput.files?.length || 0) > 0);
    }
    if (!status) return;
    const err = validateFiles(fileInput.files);
    if (err && (fileInput.files?.length || 0) > 0) {
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      status.textContent = count === 1 ? "Uploaded 1 photo." : `Uploaded ${count} photos.`;
      location.reload();
    } catch (err) {
      status.classList.add("is-error");
